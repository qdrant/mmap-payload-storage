// A copy of the bustle benchmark with a 32-byte key. :)
...
pub trait CollectionHandle {
    /// The `u32` seeds used to construct `Key` (through `From<u64>`) are distinct.
    /// The returned keys must be as well.
    type Key: From<u32> + Ord;
...
impl Workload {
    ...
    pub fn new(threads: u32, mix: Mix) -> Self {
        Self {
            mix,
            initial_cap_log2: 25,
            prefill_f: 0.0,
        ...
        <T::Handle as CollectionHandle>::Key: Send + std::fmt::Debug,
    {
        ...
        let total_ops = (initial_capacity as f64 * self.ops_f) as u32;
        ...
        let prefill = (initial_capacity as f64 * self.prefill_f) as u32;
        ...
        let max_insert_ops =
            (total_ops + 99) / 100 * u32::from(self.mix.insert + self.mix.upsert);
        let insert_keys = std::cmp::max(initial_capacity, max_insert_ops) + prefill as u32;
        ...
        let table = Arc::new(T::with_capacity(initial_capacity as usize));
        ...
        for keys in keys {
            ...
            prefillers.push(std::thread::spawn(move || {
                ...
                for key in &keys[0..prefill_per_thread as usize] {
        ...
        let barrier = Arc::new(Barrier::new(self.threads as usize + 1));
        let mut mix_threads = Vec::with_capacity(self.threads as usize);
        eprintln!("threads: {}", self.threads);
        eprintln!("keys: {:?}", keys.len());
        for keys in keys {
            eprintln!("inner keys: {:?}", keys.len());
            eprintln!("inner keys: {:?}", keys.len() as u32);
            let table = Arc::clone(&table);
