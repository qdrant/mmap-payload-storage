...
impl PageTracker {
    ...
    pub fn new(path: &Path, size_hint: Option<usize>) -> Self {
        ...
        create_and_ensure_length(&path, size as u64).unwrap();
    ...
    /// Returns defined mapping ids
    pub fn all_page_ids(&self) -> Vec<u32> {
    ...
    pub fn get(&self, offset: PointOffset) -> Option<PagePointer> {
        self.get_raw(offset).and_then(|pointer| *pointer)
    }

    pub fn get_raw(&self, offset: PointOffset) -> Option<&Option<PagePointer>> {
        let start_offset =
            size_of::<PageTrackerHeader>() + offset as usize * size_of::<Option<PagePointer>>();
        let end_offset = start_offset + size_of::<Option<PagePointer>>();
    ...
    fn persist_pointer(&mut self, point_offset: PointOffset, page_pointer: Option<PagePointer>) {
        let start =
            size_of::<PageTrackerHeader>() + point_offset as usize * size_of::<Option<PagePointer>>();
        let pointer_end_offset = start + size_of::<Option<PagePointer>>();
        // check if file is long enough
        if self.mmap.len() < pointer_end_offset {
            // flush the current mmap
            self.mmap.flush().unwrap();
            let missing_space = pointer_end_offset - self.mmap.len();
            // reopen the file with a larger size to account for missing size + default size to avoid resizing too often
            let new_size = self.mmap.len() + missing_space + Self::DEFAULT_SIZE;
            eprintln!("new size {}", new_size);
            create_and_ensure_length(&self.path, new_size as u64).unwrap();
            self.mmap = open_write_mmap(&self.path, AdviceSetting::from(Advice::Normal)).unwrap();
        }
        self.mmap[start..pointer_end_offset].copy_from_slice(transmute_to_u8(&page_pointer));
    }
    ...
    fn increment_max_point_offset(&mut self, point_offset: PointOffset) {
        if point_offset >= self.header.max_point_offset as u64 {
...
mod tests {
    ...
    fn test_persist_and_open_tracker(#[case] initial_tracker_size: usize) {
        ...
        for i in 0..value_count {
            ...
            if i % 2 == 0 {
                tracker.set(i as u64, PagePointer::new(i as u32, i as u32));
            }
        }

        assert_eq!(tracker.mapping_len(), value_count / 2);
        assert_eq!(tracker.header_count() as usize, value_count - 1);
        ...
        for i in 0..value_count {
            if i % 2 == 0 {
                assert_eq!(
                    tracker.get(i as u64),
                    Some(PagePointer::new(i as u32, i as u32))
                );
            } else {
                assert_eq!(tracker.get(i as u64), None);
            }
        }
    }
    ...
    fn test_page_tracker_resize(#[case] initial_tracker_size: usize) {
        ...
        for i in 0..100_000 {
            tracker.set(i, PagePointer::new(i as u32, i as u32));
        }
